...
// Sourced from operation: ShowSessionInfoController_run (GET /session-info/{email})
...
import { zShowSessionInfoControllerRunPath } from "../api/zod.gen.js";
const inputSchema = z.object({
  ...zShowSessionInfoControllerRunPath.shape,
});
...
export const session_info = defineTool({
  ...
  async execute(input, ctx) {
    ...
    const sdkFn = showSessionInfoControllerRun as (opts: any) => Promise<any>;
    const response = await sdkFn({
      client: sdkClient,
      path: zShowSessionInfoControllerRunPath.parse(inp),
    });
